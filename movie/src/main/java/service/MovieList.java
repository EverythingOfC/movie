...
public class MovieList implements Action {
	...
	public void execute(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(search==null) {	// 검색 조건이 없는 경우
			pDto.setTotalCount(mDao.allCount(search));	// 게시글 총 개수 리턴
			lMdto = mDao.selectAllPaging(curr,search);
		}else {
			pDto.setTotalCount(mDao.allCount(condition));	// 게시글 총 개수 리턴
			lMdto = mDao.selectAllPaging(curr,condition);
		}
		...
		request.setAttribute("search", search);	// 검색 값 저장
